...
/**
 * Staff password gate for the kitchen board. Posts to /api/kitchen/login,
 * which verifies server-side and sets the httpOnly session cookie; on success
 * we re-run the server guard (router.refresh) which then renders the board.
 */
export default function KitchenLogin() {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                router.refresh();
                return; // keep the spinner until the board takes over
            }
